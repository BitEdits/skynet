...
static char *build_key_path(const char *node_name, const char *suffix) {
    ...
    size_t path_len = strlen(dir_path) + strlen(hash_str) + strlen(suffix) + 2; // +1 for '/', +1 for '\0'
...
int main(int argc, char *argv[]) {
    srand(time(NULL)); /* Seed rand() for seq_no */
    if (argc != 4) {
    ...
    SkyNetMessage msg;
    skynet_init(&msg, SKYNET_MSG_CHAT, 0, SKYNET_NPG_CHAT, SKYNET_QOS_CHAT); /* node_id=0 since not used */
    fprintf(stderr, "Debug: Initialized SkyNetMessage\n");
    ...
    fprintf(stderr, "Debug: Writing %d bytes to stdout\n", len);
    if (fwrite(buffer, 1, len, stdout) != len) {
        fprintf(stderr, "Failed to write serialized message to stdout\n");
        return 1;
    }
    fflush(stdout);
